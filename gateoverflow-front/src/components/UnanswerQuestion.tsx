...
const UnanswerQuestion = ({i,val}:any) => {
  return (
    <div
    key={i}
    className=""
  ...
  >
    <Link to={`/questions/${val._id}`}>
      <div className="title-query-section flex justify-between p-3 xs:pb-1">
        <div className="w-full xs:max-h-[64px] overflow-hidden">
          ...
          {/* dynaic html string */}
          <div
            className="outofeditor dynamic-element-question py-1 cursor-pointer "
            id={`question${i}`}
          ></div>
        </div>
    ...
    <div className="px-3 py-1 xs:pt-0 flex items-center">
      <div className="mr-2">
        <img
          src={val.userimage}
          alt=""
          className="rounded-full w-[50px] h-[50px] xs:w-[30px] xs:h-[30px]"
        />
      </div>
      ...
      <div className="scroll-tags flex sm:hidden xs:hidden mb-[-8px]">
        {val.tags.split(",").map((tag: String, i: any) => {
          return (
            <div
              key={i}
              className="mx-1 text-gray-400 font-sans font-medium text-[15px] bg-gray-800 px-5 pb-1 rounded-full "
            >
              {tag}
            </div>
          );
